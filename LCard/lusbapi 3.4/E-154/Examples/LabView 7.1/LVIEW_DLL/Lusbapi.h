   ...
   #define RESERVED_SYNC_154	2
	...
	struct ILE154 : public ILUSBBASE
	{
      ...
      virtual BOOL WINAPI FIFO_STATUS(DWORD *FifoOverflowFlag, double *FifoMaxPercentLoad, DWORD *FifoSize, DWORD *MaxFifoBytesUsed) = 0;
		...
		// функции для работы с пользовательской информацией ППЗУ
		virtual BOOL WINAPI ENABLE_FLASH_WRITE(BOOL IsUserFlashWriteEnabled) = 0;
		virtual BOOL WINAPI READ_FLASH_ARRAY(unsigned char * const UserFlash) = 0;
		virtual BOOL WINAPI WRITE_FLASH_ARRAY(unsigned char * const UserFlash) = 0;
...
//				EXT_ADC_START_ON_HIGH_LEVEL_E2010, EXT_ADC_START_ON_LOW_LEVEL_E2010,		// для Rev.B и выше (пока нет)
	...
	enum {
				...
				OVERFLOW_OF_CHANNEL_1_E2010 = 0x0, OVERFLOW_OF_CHANNEL_2_E2010,	// локальные признаки переполнения разрядной сетки соответствующего канала
				OVERFLOW_OF_CHANNEL_3_E2010, OVERFLOW_OF_CHANNEL_4_E2010,			// за время выполнения одного запроса сбора данных ReadData()
	         OVERFLOW_E2010 = 0x7					// глобальный признак факта переполнения разрядной сетки модуля за всё время сбора данных от момента START_ADC() до STOP_ADC()
		  };
	...
	enum 	{
				...
				USER_FLASH_SIZE_E2010 = 0x200,  		// размер области пользовательско1го ППЗУ в байтах
	...
	struct ADC_PARS_E2010
	{
		...
		double InterKadrDelay;				// межкадровая задержка в мс
		double KadrRate;						// частота кадра в кГц
		double AdcOffsetCoefs[ADC_INPUT_RANGES_QUANTITY_E2010][ADC_CHANNELS_QUANTITY_E2010];	// корректировочные коэффициенты смещение	АЦП: (3 диапазона)*(4 канала) (для Rev.B и выше)
		double AdcScaleCoefs[ADC_INPUT_RANGES_QUANTITY_E2010][ADC_CHANNELS_QUANTITY_E2010];		// корректировочные коэффициенты масштаба АЦП: (3 диапазона)*(4 канала) (для Rev.B и выше)
	};
	// структура с информацией о текущем состоянии процесса сбора данных
	struct DATA_STATE_E2010
	{
		BYTE ChannelsOverFlow;			// битовые признаки перегрузки входных аналоговых каналов модуля
		BYTE BufferOverrun;				// битовые признаки переполнения внутреннего буфера модуля
		DWORD CurBufferFilling;			// заполненность внутреннего буфера модуля, в отсчётах
		DWORD MaxOfBufferFilling;		// за время сбора максимальная заполненность внутреннего буфера модуля, в отсчётах
		DWORD BufferSize;					// размер внутреннего буфера модуля, в отсчётах
		double CurBufferFillingPercent;		// текущая степень заполнения внутреннего буфера модуля, в %
		double MaxOfBufferFillingPercent;	// за время сбора максимальная степень заполнения внутреннего буфера модуля, в %
	};
	#pragma pack()
	...
	struct ILE2010 : public ILUSBBASE
	{
		...
		virtual BOOL WINAPI LOAD_MODULE(PCHAR const FileName = NULL) = 0;
		virtual BOOL WINAPI TEST_MODULE(WORD TestModeMask = NULL) = 0;

...
// указатель на интерфейс модуля E-154
ILE154 *pModule;
...
void __fastcall TMainForm::FormCreate(TObject *Sender)
{
	...
	ZS.Set(0, ChannelPoint, -2048, 2048, -2048, 2048, 6, "%-6.0f");
	...
	if(InputMode == ANALOG_SYNC_E154) DisableAdPars(false);
...
void __fastcall TMainForm::OnLoadUsbDevice(TMessage& Message)
{
	...
	if(!pModule) { Application->MessageBox("Не могу получить интерфейс на модуль E-154!", "ОШИБКА!!!", MB_OK + MB_ICONINFORMATION); Close(); return; }

	// попробуем обнаружить модуль E-154 в первых MAX_VIRTUAL_SLOTS_QUANTITY_LUSBAPI виртуальных слотах
	for(i = 0x0; i < MAX_VIRTUAL_SLOTS_QUANTITY_LUSBAPI; i++) if(pModule->OpenLDevice(i)) break;
	if(i == MAX_VIRTUAL_SLOTS_QUANTITY_LUSBAPI) { Application->MessageBox("Не могу обнаружить модуль 'E-154' в первых 127 виртуальных слотах!", "ОШИБКА!!!", MB_OK + MB_ICONINFORMATION); Close(); return; }
	...
	else if(StrComp(ModuleName, "E154")) { Application->MessageBox("В нулевом виртуальном слоте не модуль 'E-154'!", "ОШИБКА!!!", MB_OK + MB_ICONINFORMATION); Close(); return; }
	// прочитаем ППЗУ модуля
	else if(!pModule->GET_MODULE_DESCRIPTION(&ModuleDescription)) { Application->MessageBox("Не могу прочитать ППЗУ в модуле E-154!", "ОШИБКА!!!", MB_OK + MB_ICONINFORMATION); Close(); return; }
	// в ППЗУ прописано название модуля?
	else if(strcmp(ModuleDescription.Module.DeviceName, "E-154"))  { Application->MessageBox("В ППЗУ не прописано название модуля!", "ОШИБКА!!!", MB_OK + MB_ICONINFORMATION); Close(); return; }
	...
	for(i = 0x0; i < ap.ChannelsQuantity; i++) ap.ControlTable[i] = (WORD)(i)/*усиление 1*/;
...
void __fastcall TMainForm::InputModeComboBoxChange(TObject *Sender)
{
	...
	if(InputModeComboBox->ItemIndex == ANALOG_SYNC_E154) DisableAdPars(false);
	else DisableAdPars(true);
...
void __fastcall TMainForm::SynchroAdPorogTrackBarChange(TObject *Sender)
{
   ...
   if(SynchroAdPorog > 2047) SynchroAdPorog = 2047;
   else if(SynchroAdPorog < -2048) SynchroAdPorog = -2048;
	...
	if(!isdigit(Key))
		if(Key != VK_BACK	)
				...
				if(Key != '.')
            {
				   ...
				   if(SynchroAdPorog > 2047) SynchroAdPorog = 2047;
				   else if(SynchroAdPorog < -2047) SynchroAdPorog = -2047;

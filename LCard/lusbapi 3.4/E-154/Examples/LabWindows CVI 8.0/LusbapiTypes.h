...
//------------------------------------------------------------------------------
	...
	{
		struct	VERSION_INFO_LUSBAPI FwVersion;						// информация о версии прошивки основной программы 'Приложение'(Application) микроконтроллера
		struct VERSION_INFO_LUSBAPI BlVersion;						// информация о версии прошивки 'Загрузчика'(BootLoader) микроконтроллера
	};
	...
	struct DSP_INFO_LUSBAPI
	{
		...
		struct VERSION_INFO_LUSBAPI Version;							// информация о драйвере DSP
		BYTE	Comment[COMMENT_LINE_LENGTH_LUSBAPI];		// строка комментария
	};
	...
	struct MCU_INFO_LUSBAPI
	{
		...
		struct VERSION_INFO_LUSBAPI Version;							// информация о версии прошивки микроконтроллера
//		VersionType Version;										// информация о версии как самой прошивки микроконтроллера, так, возможно, и прошивки 'Загрузчика'
	...
	struct PLD_INFO_LUSBAPI										// PLD - Programmable Logic Device
	{
		...
		struct VERSION_INFO_LUSBAPI Version;							// информация о версии прошивки ПЛИС
		BYTE	Comment[COMMENT_LINE_LENGTH_LUSBAPI];		// строка комментария
	};

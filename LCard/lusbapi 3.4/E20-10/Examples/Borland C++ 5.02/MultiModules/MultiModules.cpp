...
void main(void)
{
	...
	for(i = 0x0; i < MaxVirtualSoltsQuantity; i++)
	{
		...
		// лишний раз убедимся, что это E20-10
		...
		// загрузим штатный код для ПЛИС из ресурсов Lusbapi.dll
...
//				if(!WaitingForIoRequestCompleted(IoReq[RequestNumber^0x1].Overlapped, ti)) break;
				if(WaitForSingleObject(IoReq[RequestNumber^0x1].Overlapped->hEvent, IoReq[RequestNumber^0x1].TimeOut) == WAIT_TIMEOUT) { ReadThreadErrorNumber[ti] = 0x3; break; }
				if(IsReadThreadTerminated[ti]) break;

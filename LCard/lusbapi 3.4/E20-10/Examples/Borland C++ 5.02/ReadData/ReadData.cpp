...
// Ввод осуществляется с первый четырех каналов АЦП на частоте работе АЦП 5000 кГц
//******************************************************************************
#include <stdio.h>
#include <conio.h>
#include "Lusbapi.h"
...
//	WORD DacSample;
	...
	// код для ПЛИС возьмём из соответствующего ресурса штатной DLL библиотеки
	if(!pModule->LOAD_MODULE()) AbortProgram(" LOAD_MODULE() --> Bad\n");
//	if(!pModule->LOAD_MODULE("e2010test.pld")) AbortProgram(" LOAD_MODULE() --> Bad\n");
	...
	// разрешим цифровые выходные линии
	if(!pModule->ENABLE_TTL_OUT(TRUE)) AbortProgram(" ENABLE_TTL_OUT() --> Bad\n");
	else printf(" ENABLE_TTL_OUT() --> OK\n");

//	if(!pModule->TTL_OUT(0x000F)) AbortProgram(" Ошибка установки выходных цифровых линий");
...
//	ap.SynchroPars.SynchroAdMode = ANALOG_SYNCHRO_ON_HIGH_LEVEL_E2010;
	...
	ap.ChannelsQuantity = CHANNELS_QUANTITY; 						// четыре активных канала (для Rev.B и выше)
...
//	DacSample = 0x1000;
	...
	{
		if(OldCounter != Counter) { printf(" Counter %8u  from %8u\r", Counter, NDataBlock); OldCounter = Counter; }
		else Sleep(20);
//		if(!pModule->TTL_OUT(0xFFFF)) printf(" TTL_OUT() --> Bad              \n");
//		Sleep(100);
...
DWORD WINAPI ServiceReadThread(PVOID /*Context*/)
{
	...
	OVERLAPPED ReadOv[2];
	// структуры для запросов сбора данных   
	...
	for(i = 0x0; i < 0x2; i++)
	{
		...
		IoReq[i].TimeOut = DataStep/ap.KadrRate + 2000;
	}
...
//	Sleep(1100);
	...
	if(!CancelIo(pModule->GetModuleHandle())) { ReadThreadErrorNumber = 0x9; }
...
void ShowThreadErrorMessage(void)
{
	switch(ReadThreadErrorNumber)
	{
		...
		case 0x7:
			printf(" ADC Thread: GET_DATA_STATE() --> Bad\n");
			break;

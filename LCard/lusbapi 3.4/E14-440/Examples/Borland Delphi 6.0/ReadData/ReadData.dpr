...
//             для сбора данных c модуля E14-440
//------------------------------------------------------------------------------
...
begin
	...
	// установим желаемые параметры ввода данных с модуля E14-440
...
//	for i:=0 to (ap.ChannelsQuantity-1) do ap.ControlTable[i] := i or (ADC_INPUT_RANGE_10000mV_E440 shl $6);
	for i:=0 to (ap.ChannelsQuantity-1) do ap.ControlTable[i] := i or (ADC_INPUT_RANGE_2500mV_E440 shl $6);
	ap.AdcRate := AdcRate;						// частота ввода данных в кГц
	ap.InterKadrDelay := 0.0;					// межкадровая задержка - пока всегда устанавливать в 0.0
	ap.AdcFifoBaseAddress := $0;  			// базовый адрес FIFO буфера АЦП в DSP модуля
	ap.AdcFifoLength := $3000;	 				// длина FIFO буфера АЦП в DSP модуля
	// будем использовать фирменные калибровочные коэффициенты, которые храняться в ППЗУ модуля E14-440
	...
	if ap.IsCorrectionEnabled = TRUE then WriteLn('     Data Correction is ENABLED')
	else WriteLn('       Data Correction is DISABLED');
